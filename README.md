...
Это главная страница, которая возвращает список url адресов всех методов в формате `JSON`.
Если зайти на главную страницу с браузера, то поддерживается красивое отображение `JSON` ответа. Также там находится форма для `POST` запроса, чтобы добавить новый url.

Для вызова метода необходимо добавить название метода к адресу сервера.
Например, для отображения всех полетов необходимо обратиться к адресу: http://127.0.0.1:8000/flights.getAll
